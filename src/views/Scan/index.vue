<template>
  <div class="scan-wrap">
    ...
    <div class="table-box">
      ...
      <el-table :row-style="{height:'32px',textAlign: 'center',padding:'0px',}" :cell-style="{padding:'0px',textAlign: 'center'}" border stripe ref="multipleTable" :data="tableData" tooltip-effect="dark" :style="{width: '100%',margin:'0 auto',}" :header-cell-style="getRowClass" @selection-change="handleSelectionChange" > <el-table-column type="selection" width="55"></el-table-column> <el-table-column label="序号编码" width="120"> <template slot-scope="scope">{{ scope.row.coding }}</template> </el-table-column> <el-table-column prop="name" label="样本名称" width="120"></el-table-column> <el-table-column prop="address" label="位置信息" show-overflow-tooltip></el-table-column> <el-table-column prop="status" label="状态" show-overflow-tooltip></el-table-column> <el-table-column prop="info" label="详细信息" show-overflow-tooltip></el-table-column> <el-table-column fixed="right" label="操作" width="100"> <template slot-scope="scope"> <el-button @click="newMaskTran=true" type="text" size="small">新建</el-button> <el-button @click="reMask(scope.row,scope.$index)" type="text" size="small">修改</el-button> </template> </el-table-column> </el-table>
    </div>
    <!-- ↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓  底部按钮 -->
   ...
   <!-- 新建样本 -->
    ...
    <!-- 修改样本 -->
    <transition name="el-fade-in-linear">
      <maskTran v-if="reMaskTran" :rgba="0">  <!-- rgba:透明度 -->
        <reSample title="修改样本" @changeSave="changeSave" @goBack="reMaskTran=false"></reSample>
      </maskTran>
    </transition>
    <!-- 转运 -->
    <transition name="el-fade-in-linear">
      <maskTran v-if="togegleZhuanYun" :rgba="0">  <!-- rgba:透明度 -->
...
export default {
  ...
  data () {
    return {
      ...
      // 底部按钮组的 图片
      botGroupPic: [
        { pic: require('@/assets/img/scan_bot_out.png'), link: 'loan', title: '借出' },
        { pic: require('@/assets/img/scan_bot_in.png'), link: 'guihuan', title: '归还' },
        { pic: require('@/assets/img/scan_bot_laji.png'), link: 'laji', title: '销毁' },
        { pic: require('@/assets/img/scan_bot_tie.png'), link: 'biaoqian', title: '标签' },
